 ...
 * commit.
 *
 * The pin IS the commit (catalogue format v3) and every file is fetched AT
 * that SHA — fail-closed: anything else refuses the fetch, because content
 * from a mutable ref could differ from what was curated (a moved tag would
 * be vendored silently otherwise).
 *
...
/**
 * The commit the pin stands for. Since catalogue format v3 the ref IS the
 * commit — anything else is refused, never resolved: fetching by a name
 * would vendor whatever it points at TODAY. Async for its callers' sake;
...
export async function resolvePinnedCommit(ref: AddonPackageRef): Promise<string> {
    ...
    throw new PackageFetchError(
        `„${ref.ref}" ist kein Commit-Pin — ohne eindeutigen Stand wird nichts installiert.`,
        502,
    )
}
